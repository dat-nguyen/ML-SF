...
def rank_simple(vector):
    values = [abs(float(x)) for x in vector]
    ranges = list(range(len(vector)))
#    zip(*sorted(zip(values, ranges)))
    return sorted(ranges, key=values.__getitem__)

# \TODO: doesn't work with python3
